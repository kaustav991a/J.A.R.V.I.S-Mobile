...
2. **6 — the transcribe prompt overcorrects on plain English.** `_GEMINI_TRANSCRIBE_PROMPT` (`cloud_gateway.py:901`) primes for romanised Bengali by design, and a plain English question came back as *"ki service"* — answered correctly for the question it heard, which is worse than a failure because it looks like an answer. The audio path itself is proved: `brains.usage.audio` reached `gemini_ok` with no fallback on 2026-08-27.
3. **14 — the situation on the persona envelope.** `where` already rides on it (`:4107`); place, battery and link as one field is the rest. The most character per line of code left anywhere in the plan.
4. **12, the second half — retire the master token**, once `master_calls` has gone quiet. Not before, and the counter is the evidence rather than the calendar.
5. **11 — nothing is owed here any more.** The listener was gated on the token split and the gateway half is live, so the gate is open; what remains is a service registration and a runtime permission, which is app work behind a native build.
