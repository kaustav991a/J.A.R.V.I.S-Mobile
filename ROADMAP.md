...
**59 of 91 rows are proved on the phone** (65%). 76 have code (84%). 23 cannot be finished in this repo: 19 on the brain, 2 on the desk, 2 on the phone.
...
**PROVED 2026-09-02 evening and 09-03 morning, on the one question that was left: two circles 150 m apart.** The walk home named three overlapping neighbours in order, eight minutes apart — **Mousumi's Home 9:21 PM, Musalman Para 9:26 PM, Laxminath Nagar 9:29 PM** — and the walk out the next morning named the same three in reverse: **Home 8:07, Laxminath Nagar 8:10, Musalman Para 8:11.**

A coherent sequence in both directions is a better answer than a screenshot of a header could ever be: if the radius were wrong the order would scramble or a name would repeat, and if the accuracy guard were too tight the middle ones would simply be missing — which is exactly what happened while the sweep rule was eating them. |
...
**The evening of 2026-09-02 was spent on false positives, and two of the fixes were worse than the bug.**

1. **A single drifting fix.** *"Left Office, 6:12 PM"* from a desk he had not left. The map caught it live: between two screenshots a minute apart the dot moved outside the circle with the accuracy ring ballooning. One place, so no burst — the sweep rules had nothing to compare it against. An exit now takes **one fresh reading** and compares it to the place it claims was left; every doubt resolves in favour of the crossing, because the geofence is the measurement and the fix is only a witness that can say *he is still here*.
2. **A race between two sweep exits.** *"Left Mousumi's Home"* and *"Left Sealdah Rail Station"* a second apart, from an office forty kilometres from either. Each handler read the store before the other had written, so neither saw a burst. The ten-second wait was already there and was checking the wrong thing — only whether its own sighting survived. It re-runs the whole check now.
3. **And the rule that ate the data.** *You can only leave where you were* judged its evidence over six hours, so every leg of a commute was contradicted by the leg before it. Home 8:06, Barrackpore 8:41, Sealdah 9:31, Sector V 10:03 — four true departures, three deleted at the next launch. Ten minutes now.

**The crossing store is empty as of tonight** — the deletions plus a round of dismissals by hand took Monday's 7:08 PM with them. Nothing else was touched: twelve weeks of app-open sightings, the chat log, the journal and the facts are all intact, because no rule here can reach anything without a `via`. Not rebuilt from the notification log on purpose — **writing invented rows to make a counter look better is the failure this project keeps naming**, and tomorrow regenerates the same four for free.

**2026-09-02 evening: the whole journey home, correct, after a day of fixes.** Left Office 7:07 PM · Sector V Metro 7:21 · Sealdah Rail 8:12 · Mousumi's Home 9:21 · Musalman Para 9:26 · Laxminath Nagar 9:29. Six departures, right order, overlapping neighbours included, **no false ones** — and the 7:07 PM office departure is the second measured one against a man who says he leaves at seven.

**09-03 morning did it again outbound:** Home 8:07 · Laxminath Nagar 8:10 · Musalman Para 8:11 · Barrackpore Railway Station 8:47 · Sealdah Rail Station 9:30.

So the four rules written under fire on 09-02 all hold at once: distance telling overlapping circles from a platform sweep, the ten-minute contradiction window that stopped deleting commutes, the fresh-fix second opinion against drift, and the burst race. **Every one of them was found by him reading a sentence the app wrote.**

**Checked 09-03: the delivery lag is cosmetic, and the recorded times are not.** He noticed some notifications only appearing when he opened the app, which is the one observation that could have meant the whole feature was still measuring *when you looked at your phone*. Android records a post time per notification and the body carries the figure the handler computed, so the two can be compared:

\| Place \| Recorded \| Posted \|
\| --- \| --- \| --- \|
\| Home \| 8:07 \| 8:07 \|
\| Laxminath Nagar \| 8:10 \| 8:10 \|
\| Musalman Para \| 8:11 \| 8:11 \|
\| Barrackpore \| 8:47 \| **8:57** \|
\| Sealdah \| 9:30 \| **9:32** \|

**The handler ran on time in every case; only the display lagged**, by ten minutes on one and two on another. The store holds the handler figure, so the data is right and two notifications were simply seen late.

**The standing limitation, written down so nobody re-derives it:** the recorded time is when Play Services *reported* the crossing, which is itself two to five minutes after the boundary was crossed. Every figure is therefore late by a consistent few minutes in the same direction, which a median handles. Expo does not expose the event's own `triggeringLocation` timestamp; if it ever does, that is the figure to prefer.

**The Crossings row read back the whole morning on 09-03, and it closes the loop from boundary to store to screen:**

> Reached Office · 10:04 AM · Left Sector V Metro 10:03 · Reached Sector V 9:55 · Left Sealdah Rail 9:30 · Reached Sealdah Rail 9:23 · Left Barrackpore Railway 8:47

A journey that reads correctly as data: thirty-six minutes on the train, seven on the platform at Sealdah, the metro across, and at the desk at 10:04. **Both directions of every boundary, no duplicates, no phantoms, and the times match the notification shade exactly.** *No bursts refused today.*

**`Reached Office 10:04 AM` is the figure that matters next**: the arrival hour the app has never had. Four days of those and `arrivalHour` stops quoting *"usually you are there by 11:51 AM"* — a median of app-opens by a man at his desk since ten.
...
Plan: `docs/superpowers/plans/2026-09-03-sightings-to-sqlite.md`. 1,485 tests, runtime unmoved at `5f318efe`, published as `601a95f0`. Runtime unmoved at `5f318efe`, published as `601a95f0`.

**PROVED on `84f40716`, 2026-09-03 17:44.** Places → Crossings recorded reads **"138 sightings held, reaching back 12 days"**, above six real crossings from this morning's commute — Barrackpore 8:29/8:47, Sealdah 9:23/9:30, Sector V 9:55, Office 10:04 — and *"6 bursts refused today"*. The migration ran; the diagnostic row was the whole reason it could be said either way.

**And the figure corrects something this project has been asserting all week.** Every note about the keystore says an uninstall would cost *"twelve weeks of sightings"*. It would cost **twelve days**. Twelve weeks was the TTL — the cap the old blob filtered to on every read — and it was mistaken for the contents; the store has never actually held more than the fortnight since the habit feature was built. The real cost of an uninstall is the journal's 52,000 usage moments and the chat archive, not the sightings.

That makes the import (queue 27) worth more, not less: `anticipate-habit` wants four distinct days and there are twelve in total, most of them app-open bounds rather than measured crossings. |
...
**Three false departures reached the shade on 09-02 and all three are now answered** — a drifting fix, a race between two sweep exits, and the six-hour contradiction rule. Each was caught by him reading a notification and saying *but I am at my desk*, which remains the only test that has ever found one of these.

**Two clean commutes since the fixes, 09-02 evening and 09-03 morning — eleven departures, no false ones.** The notification that mattered most is the plainest: *"Left Office · 7:07 PM."* |
...
**09-03: the data it was waiting for is now arriving in the right shape.** Two measured office departures (7:08 and 7:07 PM) and the first measured arrival (**Reached Office 10:04 AM**). `leftBy` wants four distinct days and `arrivalHour` the same, so both turn over early next week with no code — and the figure that replaces *"usually you are there by 11:51 AM"* will be an hour and three quarters earlier, because the old one was a median of app-opens by a man at his desk since ten. |
...
**The map earned its place on 2026-09-02 by diagnosing a bug.** Two screenshots a minute apart showed the dot leaving the Office circle while he sat still, with the accuracy ring swelling around it — which is a drifting fix, drawn. The notification said *"Left Office"* and the map said *why*. |
