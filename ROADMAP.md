...
**52 of 88 rows are proved on the phone** (59%). 72 have code (82%). 24 cannot be finished in this repo: 19 on the brain, 2 on the desk, 3 on the phone.
...
**Still `untested` until an exit is seen.** The control is on Places (WATCH), the radius is 120 m, and the first real proof is a departure whose recorded time matches the clock — which cannot be made to happen from a laptop, only by leaving. |
