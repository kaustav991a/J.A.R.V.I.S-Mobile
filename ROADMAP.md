...
**46 of 86 rows are proved on the phone** (53%). 66 have code (77%). 29 cannot be finished in this repo: 21 on the brain, 2 on the desk, 6 on the phone.
...
| Desk-key handshake (sealed turns) | untested | Desk | **Cause found and fixed 2026-08-29, unproved.** `has_desk_key: false` was read off the LIVE gateway with the desk off — which is the ordinary state of this system, since the PC being off is exactly why a fact is queued. The key was held in memory and in a file on Render's disk, so **every deploy and every free-tier spin-down returned the gateway to the one state where `queue_fact` cannot seal and drops the turn instead.** Eighteen went that way in a week. The desk's PUBLIC half and the sealed queue now mirror into the same Postgres `gateway_state` table the schedule uses, injected as a hook rather than imported so `fact_outbox` keeps its stdlib-only discipline; what crosses is ciphertext and one public key. `restore()` is additive, so it cannot race the container's own disk or drop a fact queued while the database was still answering. 44 checks in `test_fact_transport.py`, including a fixture that wipes memory AND disk the way a deploy does. **Owed: the deploy, the desk on once, and then `has_desk_key` still true after the deploy AFTER that.** |
